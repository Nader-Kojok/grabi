...
import { getSubcategoryBySlug } from '../utils/categories';
...
const SubcategoryPage: React.FC = () => {
  ...
  const result = categorySlug && subcategorySlug 
      ...
      <>
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-3xl font-bold text-gray-900 mb-4">Sous-catégorie introuvable</h1>
            <p className="text-gray-600 mb-6">La sous-catégorie que vous recherchez n'existe pas.</p>
  ...
  // Filter mock listings by subcategory (for demo purposes)
  const subcategoryListings = mockCurrentListings.filter(
    listing => listing.category.toLowerCase().includes(subcategory.name.toLowerCase().split(' ')[0])
  );
        ...
        <div className="bg-white border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <nav className="flex items-center space-x-2 text-sm">
              ...
              <Link to={`/category/${category.slug}`} className="text-gray-500 hover:text-gray-700">
                {category.name}
              </Link>
              <ChevronRight className="h-4 w-4 text-gray-400" />
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <Link to={`/category/${category.slug}`} className={`text-sm ${category.color} hover:underline`}>
                    {category.name}
                  </Link>
                  <ChevronRight className="h-4 w-4 text-gray-400" />
                </div>
                ...
                <p className="text-gray-600 mt-1">
                  {subcategoryListings.length} annonce{subcategoryListings.length > 1 ? 's' : ''} disponible{subcategoryListings.length > 1 ? 's' : ''}
                </p>
              </div>
            </div>
              ...
              {category.subcategories
                ...
                .map((sub) => (
                  <Link
                    ...
                    className="px-4 py-2 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors duration-200 text-sm font-medium text-gray-700 hover:text-gray-900 border border-gray-200 hover:border-gray-300"
                  >
                    {sub.name}
                  </Link>
                ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {subcategoryListings.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-600 text-lg">Aucune annonce disponible dans cette sous-catégorie pour le moment.</p>
